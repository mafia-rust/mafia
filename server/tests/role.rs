...
use mafia_server::game::{components::{cult::CultAbility, insider_group::InsiderGroupRef}, role::{armorsmith::Armorsmith, drunk::Drunk, ojo::Ojo, recruiter::Recruiter, scarecrow::Scarecrow, tally_clerk::TallyClerk, warper::Warper}, role_list::RoleSet};
...
fn cult_alternates() {
    ...
    game.next_phase();
    assert!(b.alive());
    assert!(InsiderGroupRef::Cult.is_player_in_revealed_group(game.deref(), b.player_ref()));
    ...
    assert!(!c.alive());
    assert!(d.alive());
    assert!(!InsiderGroupRef::Cult.is_player_in_revealed_group(game.deref(), d.player_ref()));
