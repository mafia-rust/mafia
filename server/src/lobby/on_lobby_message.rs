...
use super::{lobby_client::{LobbyClient, LobbyClientType, Ready, RoomClientID}, lobby_state::Lobby, name_validation::{self, sanitize_server_name}, RemoveClientData};
...
impl Lobby {
    pub fn on_client_message(&mut self, send: &ClientSender, lobby_client_id: RoomClientID, incoming_packet: ToServerPacket) -> LobbyAction {
        'packet_match: { match incoming_packet {
            ToServerPacket::SendLobbyMessage { text } => {
                ...
                ) = self.clients.get(&lobby_client_id) {
                    name.clone()
                } else {
                    break 'packet_match
                };
            ...
            ToServerPacket::SetName{ name } => {
                self.set_player_name(lobby_client_id, name);
            },
            ...
            ToServerPacket::SetLobbyName{ name } => {
                if !self.is_host(lobby_client_id) {break 'packet_match};
            ...
            ToServerPacket::StartGame => {
                ...
                for (lobby_client_id, lobby_client) in self.clients.clone() {
