...
impl MafiaRecruits{
    pub fn recruit(game: &mut Game, player: PlayerReference)->bool{
        ...
        if InsiderGroupRef::Mafia.is_player_in_revealed_group(game, player) {return false;}
        if !recruiter_recruits.recruits.insert(player){return false;}

        game.set_recruiter_recruits(recruiter_recruits);
        InsiderGroupRef::Mafia.add_player_to_revealed_group(game, player);
    ...
    pub fn give_tags_and_labels(game: &mut Game){
        for player_a in InsiderGroupRef::Mafia.players(game).clone() {
    ...
    pub fn mafia_members(game: &Game)->HashSet<PlayerReference>{
        PlayerReference::all_players(game)
            .filter(|p|InsiderGroupRef::Mafia.is_player_in_revealed_group(game, *p))
            .map(|p|p.clone())
            .collect()
    }
