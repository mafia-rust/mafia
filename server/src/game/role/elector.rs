...
use crate::game::modifiers::elector_and_president::{ElectorAndPresident, President};
use crate::game::modifiers::{ModifierState, Modifiers};
...
impl RoleStateImpl for Drunk {
    ...
    fn before_initial_role_creation(self, game: &mut Game, actor_ref: PlayerReference) {
        Modifiers::set_modifier_state(game, ModifierState::ElectorAndPresident(ElectorAndPresident{
            elector: actor_ref,
...
impl Drunk{
    const POSSIBLE_ROLES: [Role; 6] = [
        Role::Detective, Role::Snoop, Role::Gossip, Role::Philosopher, Role::Psychic, Role::TallyClerk
    ];
}