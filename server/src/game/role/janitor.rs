...
pub(super) fn do_night_action(game: &mut Game, actor_ref: PlayerReference, priority: Priority) {
    ...
    let RoleData::Janitor { cleans_remaining, .. } = actor_ref.role_data(game) else {unreachable!()};
    ...
    match priority {
        ...
        Priority::Investigative=>{
            let RoleData::Janitor { cleaned_ref, .. } = actor_ref.role_data(game) else {unreachable!()};
...
pub(super) fn on_phase_start(game: &mut Game, actor_ref: PlayerReference, _phase: PhaseType){
            let &RoleData::Janitor { cleans_remaining, .. } = actor_ref.role_data(game) else {unreachable!()};
    actor_ref.set_role_data(game, RoleData::Janitor { cleans_remaining, cleaned_ref: None });
}
