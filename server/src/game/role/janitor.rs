...
pub(super) fn do_night_action(game: &mut Game, actor_ref: PlayerReference, priority: Priority) {
    ...
    let RoleState::Janitor { cleans_remaining, .. } = actor_ref.role_data(game) else {unreachable!()};
    ...
    match priority {
        Priority::Deception=>{
            ...
            if target_ref.night_jailed(game) {
                ...
                actor_ref.set_role_data(game, RoleState::Janitor { cleans_remaining: cleans_remaining-1, cleaned_ref: Some(target_ref) });
            }
        },
        Priority::Investigative=>{
            let RoleState::Janitor { cleaned_ref, .. } = actor_ref.role_data(game) else {unreachable!()};
...
pub(super) fn on_phase_start(game: &mut Game, actor_ref: PlayerReference, _phase: PhaseType){
            let &RoleState::Janitor { cleans_remaining, .. } = actor_ref.role_data(game) else {unreachable!()};
    actor_ref.set_role_data(game, RoleState::Janitor { cleans_remaining, cleaned_ref: None });
}
