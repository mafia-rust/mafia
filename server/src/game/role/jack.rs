...
impl RoleStateImpl for Jack {
    ...
    fn on_midnight(self, game: &mut Game, midnight_variables: &mut MidnightVariables, actor_ref: PlayerReference, priority: OnMidnightPriority) {
        let Some(&RoleOptionSelection(Some(role))) = ControllerID::role(actor_ref, Role::Jack, 0)
            .get_role_option_selection(game) else {return};
        match (priority, role) {
            (OnMidnightPriority::Investigative, Role::Snoop) => {
                let actor_visits = actor_ref.untagged_night_visits_cloned(game);
                let Some(visit) = actor_visits.first() else {return};
                let townie = actor_ref.all_night_visitors_cloned(game).is_empty() &&
    ...
    fn controller_parameters_map(self, game: &Game, actor_ref: PlayerReference) -> ControllerParametersMap {
        let mut controller = ControllerParametersMap::builder(game)
    ...
    fn convert_selection_to_visits(self, game: &Game, actor_ref: PlayerReference) -> Vec<Visit> {
        let Some(type_selection) = ControllerID::role(actor_ref, Role::Jack, 0)
            .get_role_option_selection(game) else {return Vec::new()};
